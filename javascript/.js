class Character {
    ...
    updateAbilities() {
        ...
        if (this.classType.toLowerCase() === "warrior" || this.classType === "1") {
            ...
            baseAbilities = {
                "Rage": { damage: Math.floor(25 * scaling), manaCost: 15, description: "Strong attack with bonus damage" },
                "Shield Block": { defense: Math.floor(15 * scaling), duration: 2, manaCost: 10, description: "Temporary defense boost" }
            };
            if (this.level >= 3) {
        ...
        } else if (this.classType.toLowerCase() === "mage" || this.classType === "2") {
...
function getTarget(enemies, auto = false) {
    ...
    if (enemies.length === 1 || auto) {
        ...
        const targetIdx = parseInt(prompt("> ")) - 1;
...
function levelUpDisplay(player, oldLevel, rewards) {
    ...
    console.log(`║${" ".repeat(16)}Level ${oldLevel} → ${player.level}${" ".repeat(21)}║`);
    console.log(`╚${"═".repeat(48)}╝`);
    console.log("\nStats increased:");
    console.log(`♥ Max HP: ${player.maxHealth - rewards.health} → ${player.maxHealth}`);
    ...
    if ([3, 5].includes(player.level)) {
        ...
        const newAbilities = Object.entries(player.abilities)
...
function combat(player, enemies) {
  console.log("\nEnemies appear!");
  ...
  while (enemies.some((enemy) => enemy.health > 0) && player.health > 0) {
    ...
    if (
      ...
      target = getTarget(enemies, auto_target);
      if (!target) {
    ...
    if (choice === "1") {
      ...
      const abilityChoice = prompt("Choose ability number (or 'back'): ");
      ...
      const itemChoice = prompt("Choose item to use (or 'back'): ");
      ...
      if (player.gadgets.length > 0) {
        ...
        if (gadgetList.length > 0) {
          const gadgetChoice = prompt("Choose gadget number (or 'back'): ");
  ...
  if (player.health > 0) {
    ...
    const totalExp = defeatedEnemies.reduce((sum, e) => sum + e.exp_reward, 0);
...
function showGadgets(player) {
  ...
  for (const [name, gadget] of Object.entries(player.gadgets)) {
    if (gadget.charges > 0) {
      ...
      console.log(`${gadgetList.length}. ${name} (${gadget.charges} charges)`);
    }
  }
  return gadgetList;
}
  ...
  if (ability.area_damage) {
    ...
    for (let hit = 0; hit < hits; hit++) {
      if (target.health > 0) {
        ...
        console.log(`Hit ${hit + 1}: ${baseDamage} damage to ${target.name}!`);
...
function processStatusEffects(entity) {
  ...
  for (const effect of [...entity.status_effects]) {
    ...
    if (effect.duration <= 0) {
      entity.status_effects = entity.status_effects.filter((e) => e !== effect);
...
function showInventoryMenu(player) {
  while (true) {
    console.log("\n=== Inventory Menu ===");
...
function currencyExchange(player) {
    ...
    if (choice === "1") {
      ...
      try {
        const amount = parseInt(
          prompt("How many tech points to buy? (0 to cancel): "),
          10
        );
      ...
      try {
        const amount = parseInt(
          prompt("How many tech points to convert? (0 to cancel): "),
          10
        );
...
function main() {
  ...
  let classChoice;
  while (true) {
    classChoice = prompt("Choose your class (1-12): ");
    if (
  ...
  while (true) {
    ...
    console.log(`EXP: ${player.exp}/${calculateExpRequirement(player.level)}`);
    console.log(`Gold: ${player.gold}`);
    ...
    if (choice === "1") {
      ...
      if (enemies.length) {
        const result = combat(player, enemies);
        if (!result) {
